...
// import { CurrencyMaskConfig, ICURRENCY_MASK_CONFIG } from './currency-mask.config';
import { InputHandler } from './input.handler';
import { NUMBER_MASK_CONFIG, INumberMaskConfig } from './contracts';
...
export class NumberMaskDirective implements AfterViewInit, ControlValueAccessor, DoCheck, OnInit, Validator {
    ...
    ngAfterViewInit() {
        this.elementRef.nativeElement.style.textAlign = this.config.align ? this.config.align : this.optionsTemplate.align;
    }

    ngDoCheck() {
        if (this.keyValueDiffer.diff(this.config)) {
            this.elementRef.nativeElement.style.textAlign = this.config.align ? this.config.align : this.optionsTemplate.align;
            this.inputHandler.updateOptions((<any>Object).assign({}, this.optionsTemplate, this.config));
        }
    }

    ngOnInit() {
        this.inputHandler = new InputHandler(this.elementRef.nativeElement, (<any>Object).assign({}, this.optionsTemplate, this.config));
    }
